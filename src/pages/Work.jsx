...
const Work = () => {
  ...
  const displayedProjects =
    activeCategory === "all"
      ? EleData
      : EleData.filter((proj) => proj.type === activeCategory);


  async function detailfetch() {
    const res = await axios.post(
      "https://cspv.in/hpotpl/oxymora-web/apis/fetch_portfolio.php",
      {
        type: "all",
      },
      {
    ...
    <div>
      ...
      <div className="Work-Contanior">
        ...
        {/* {displayedProjects.map((proj) => (
          ...
          {displayedProjects.map((ele) => (
            <Project1 key={ele.id} Data={ele} />
          ))}
